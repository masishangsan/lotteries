            ...
            <div class="content_first_content">
                <div v-for="item in message" class="content_first_content_item">
                    ...
                    <div style="display: flex">
                        <div v-for="item1 in item.blueBall" style="margin: 0 0.2rem;width: 40%">{{item1}}</div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
	export default {
		...
		methods:{
			...
			getTwo_message(){
				var self = this
				self.$axios.post('choose_getMessage',{},res=>{
					console.log(res,'message')
					res.data.forEach(function (item) {
						self.message.push({
							num:item.expect,
							redBall:item.opencode,
							blueBall:item.blue_number
						});
					});
			...
			getTwo_R(){
				var self=this
				self.$axios.post('choose_getR',{},(res)=>{
					// console.log(res)
			...
			getTwo_B(){
				var self=this
				self.$axios.post('choose_getB',{},(res)=>{
					// console.log(res,'sss')
					let data = JSON.parse(JSON.stringify(res.data))
