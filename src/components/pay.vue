<template>
    <div id="pay">
        ...
        <div class="body">
            <div class="content">
                <div class="title">请设置充值数量</div>
                <div class="box">
                    <div v-for="(item,index) in numbers" class="box_item" >
                        <div class="left" @click="select(index)">
                            <div :class="[item.choose?'select':'']">{{item.number}}</div>
                            <img src="../../static/img/jindou.png" alt="">
                        </div>
                        <div class="right">
                            <div>{{item.numberlock}}</div>
                            <img src="../../static/img/jinbi.png" alt="">
                        </div>
                    </div>
                </div>
                <div class="input"><input type="number" placeholder="自定义充值数量" v-model="jieguo"></div>
                <div class="close"></div>
                <mt-button size="large" type="danger" @click="topay" clas="close">充值</mt-button>
            </div>
        </div>
    </div>
</template>
        ...
        methods:{
	        ...
	        topay(){
	        	...
	        	if (self.jieguo<1){
			        ...
			        MessageBox.confirm('确定充值'+self.jieguo+'个金豆？').then(action => {
				        self.$axios.post('pay',{
