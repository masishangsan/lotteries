    ...
    <div class="choose" @click="menu=false">
        <div class="action">
                ...
                <div class="action_right">
                    <img src="../assets/file.png" alt=""  @click="$router.push('/record_choose')">
        ...
        <div class="ash_box" :class="{'ash_boxY': ash_box}" >
            <div class="screen_box" @click="stop($event)">
                <div class="screen_box_title">筛选条件</div>
                <div class="screen_box_content">
                    <div class="tuo">
                        <div style="color: #FF0000;margin-bottom: 0.5rem">请选择胆码（最多选择4个）</div>
                        <div class="redBall">
	...
	export default {
		...
		methods:{
			select(index){
				...
				if (self.push1.length>=5 && self.push2.length>=2){
			...
			shake(){
				var shakeArr=this.getRandomArray(this.ball, 5);
				...
				this.push1 = shakeArr;
				///////////////////////////////////////////////
				var shakeArr1=this.getRandomArray(this.balls, 2);
			...
			menuChooseF(index){
				if (index == 0){
					...
					this.$router.push({path: 'trend_choose'});
					this.menu=false
				}else if(index==2){
			...
			getTwo_N(){
				var self = this;
				self.$axios.post('choose_getNoRe',{},(res)=>{
					if (res.msg=='success'){
						self.$axios.post('choose_getNo',{},(res)=>{
			...
			getEndtime(){
				var self=this
				self.$axios.post('choose_getEndtime',{},(res)=>{
